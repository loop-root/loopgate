...
1. Audit append failure on security-relevant actions (capability execution,
   session open) is a hard failure. Do not convert to log-and-continue.

2. Do not split lock acquisitions on `server.mu` or `server.auditMu` across
   multiple lock/unlock pairs for the same logical
...
4. Secret values (`SecretValue`, raw API keys, tokens) must be extracted from
...
5. `isSecretExportCapability` is a defense-in-depth guard, not the primary
...
6. Do not add background goroutines, cleanup daemons, or autonomous lifecycle
   workers to Loopgate without an explicit design review. Loopgate is
   intentionally synchronous and request-driven.

7. Model-originated content (memory strings, status text, summaries) must not
