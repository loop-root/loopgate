...
| F-18 | In progress | `internal/auditruntime`, `internal/protocol`, and `internal/approvalruntime` now sit outside `internal/loopgate` with one-way imports. The top-level `internal/loopgate` file count is still too high, so continue extracting cohesive runtime domains. |
