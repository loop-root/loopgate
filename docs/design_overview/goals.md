...
- **Explicit boundary** — **Loopgate** is the primary product and authority for policy, capabilities, secrets, sandbox, audit, and morphlings; **operator clients** attach via **HTTP on the local Unix socket**. **In-tree MCP is deprecated and removed** (ADR 0010 — shrinks attack surface); **out-of-tree** MCP→HTTP forwarders remain possible outside this repo’s shipped surface, and a **future ADR** may reserve a thin in-tree forwarder only.
