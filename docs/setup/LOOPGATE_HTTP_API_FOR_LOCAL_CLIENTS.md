...
If a future build enables **executable path pinning** (`expectedClientPath` in server code), only the expected binary may open sessions; today’s default standalone `cmd/loopgate` does not set that in-tree.
