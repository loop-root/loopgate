#!/usr/bin/env bash
# Start Loopgate for native clients (e.g. Swift Haven in ~/Dev/Haven) over the Unix socket HTTP API.
# Does not build or run the Wails app — only Go cmd/loopgate from this Morph checkout.
#
# IMPORTANT: Run the copy of this script inside the Morph repo (e.g. ~/Dev/morph/scripts/…).
# Do not copy it into the Swift app repo: REPO_ROOT is derived from this file's location; a copy
# under ~/Dev/Haven would point at the wrong tree and Loopgate would not load config/runtime.yaml
# (diagnostic logs stay off, policy/persona paths wrong).
#
# Usage (from Morph repo):
...
#   MORPH_REPO_ROOT          — repo root for config/policy (default: directory above scripts/; also exported for cmd/loopgate)
#   LOOPGATE_SOCKET          — Unix socket path; must match your Swift client (default: $REPO_ROOT/runtime/state/loopgate.sock)
#   LOOPGATE_REUSE_RUNNING   — if 1, keep an already-ready Loopgate on the socket (default: 0)
#   HAVEN_REUSE_RUNNING_LOOPGATE — alias for LOOPGATE_REUSE_RUNNING (same as ./start.sh)
...
REPO_ROOT="${MORPH_REPO_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"
REPO_ROOT="$(cd "$REPO_ROOT" && pwd)"

if [[ ! -f "$REPO_ROOT/config/runtime.yaml" ]]; then
  echo "error: expected Morph repo at REPO_ROOT=$REPO_ROOT (missing config/runtime.yaml)." >&2
  echo "  Fix: invoke scripts/start-loopgate-swift-dev.sh from your Morph checkout, not a copy under another repo." >&2
  echo "  Or set MORPH_REPO_ROOT to the Morph tree that contains config/runtime.yaml." >&2
  exit 1
fi
