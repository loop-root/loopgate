...
func summarizeSettings(commandContext CommandContext) string {
	lines := []string{
		...
		fmt.Sprintf("morphling_spawn_enabled: %t", commandContext.Policy.Tools.Morphlings.SpawnEnabled),
		fmt.Sprintf("morphling_max_active: %d", commandContext.Policy.Tools.Morphlings.MaxActive),
		fmt.Sprintf("morphling_require_template: %t", commandContext.Policy.Tools.Morphlings.RequireTemplate),
		fmt.Sprintf("active_morphlings: %d", commandContext.LoopgateStatus.ActiveMorphlings),
