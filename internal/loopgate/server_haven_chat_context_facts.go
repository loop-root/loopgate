...
func buildHavenBaseRuntimeFacts(currentTime time.Time, providerName string, modelName string) []string {
	...
	if trimmedModelName := strings.TrimSpace(modelName); trimmedModelName != "" {
		...
		if trimmedProviderName != "" {
			identityFact += fmt.Sprintf(" If asked about your underlying model, say you are running on %s (%s).", trimmedModelName, trimmedProviderName)
		} else {
			identityFact += fmt.Sprintf(" If asked about your underlying model, say you are running on %s.", trimmedModelName)
		}
	}
